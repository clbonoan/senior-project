...
def sobel_xy(gray32): 
    ...
    gy = cv.Sovel(gray32, cv.CV_32F, 0, 1, ksize=3)
    return gx, gy
...
def calculate_light_tamper_score(
    ...
    for i in range(1, num_shadows):  # 0 = background
        ...
        features, log_ratio = features_one_shadow(
    ...
    usable = len(features_one_shadow)
    ...
    #     return 0.5, light_features  # not enough shadows to compare
    ...
    if usable > 0:
        agg("comp_area", [f['area'] for f in features_one_shadow])
        agg("boundary_mrl", [f['mrl'] for f in features_one_shadow])
        agg("boundary_angle_std", [f['angle_std'] for f in features_one_shadow])
        agg("umbra_frac", [f['umbra_frac'] for f in features_one_shadow])
        agg("ring_to_area", [f['ring_to_area'] for f in features_one_shadow])
        agg("Yn", [f['Yn'] for f in features_one_shadow])
        agg("sr_median", [f['sr_median'] for f in features_one_shadow])
        agg("sr_iqr", [f['sr_iqr'] for f in features_one_shadow])
        agg("sr_skew", [f['sr_skew'] for f in features_one_shadow])
        agg("sr_kurtosis", [f['sr_kurtosis'] for f in features_one_shadow])
    else:
    ...
    if usable >= 2:
        ...
        feature_matrix = np.array([[f['sr_median'], f['sr_iqr'], f['sr_skew'], f['sr_kurtosis']]
                                   for f in features_one_shadow], dtype=np.float32)

        # pairwise distances in Z-space (all 4 features)

# lighting.py - Shadow lighting consistency analysis
# Combines feature extraction (for ML) and rule-based scoring
...
def find_bright_nearby(img_bgr, shadow_mask, brightness, expand=2, color_diff=0.02):
    kernel = np.ones((3,3), np.uint8)
    sm = (shadow_mask > 0)
    expanded = cv.dilate(sm.astype(np.uint8), kernel, iterations=expand).astype(bool)
    ...
    avg_color = color_dirs[eroded].mean(axis=0, keepdims=True)
    
    ring_indices = np.flatnonzero(ring.ravel())
    ring_colors = color_dirs.reshape(-1,3)[ring_indices]
    
    similarity = (ring_colors @ avg_color.T).ravel()
...
def analyze_one_shadow(
    ...
    """
    Analyze a single shadow component.
    Returns: (features_dict, log_ratio_median) or (None, None) if invalid
    """
    area = int(np.count_nonzero(shadow_mask))
    
    # Boundary alignment features
    boundary = boundary_from_mask((shadow_mask > 0).astype(np.uint8) * 255)
    ys, xs = np.where(boundary > 0)
    if ys.size > 0:
        gxs, gys = gx[ys, xs].astype(np.float32), gy[ys, xs].astype(np.float32)
        ...
        ang = np.arctan2(ny, nx)
        C, S = float(np.mean(np.cos(ang))), float(np.mean(np.sin(ang)))
        mrl = float(np.sqrt(C*C + S*S))
        angle_std = float(np.sqrt(max(0.0, -2*np.log(mrl + 1e-6))))
    else:
        mrl, angle_std = 0.0, 0.0

    # Find umbra and bright reference
    dark_part = find_dark_part(shadow_mask, brightness, darkest_frac, shrink)
    if dark_part is None:
        return None, None

    bright_area = find_bright_nearby(img_bgr, shadow_mask, brightness, expand, color_diff)
    if bright_area is None or bright_area.sum() < min_bright_pixels:
        return None, None

    bright_val = float(np.median(brightness[bright_area]))
    if not np.isfinite(bright_val) or bright_val < min_bright_ref:
        return None, None

    dark_values = brightness[dark_part]
    dark_values = dark_values[np.isfinite(dark_values)]
    if dark_values.size < min_dark_pixels:
        return None, None
    ...
    if debug and viz is not None:
        overlay = img_bgr.copy()
        overlay[dark_part] = [0, 0, 255]
        overlay[bright_area] = [0, 255, 0]
        viz.add_shadow_and_bright(overlay)

    # Log ratio for shadow strength
    strength_ratios = np.log(np.maximum(dark_values, 1e-6)) - np.log(max(bright_val, 1e-6))
...
def calculate_light_tamper_score(
    ...
    """
    Analyze shadow lighting consistency.
    
    Returns:
        score: float (0-1), rule-based tamper score
        ml_features: dict of features for ML (does NOT include score)
    """
    mask_kwargs = mask_kwargs or {}
    ...
    for i in range(1, num_shadows):
        current_shadow = (labels == i)
        
        shadow_feats, log_ratio = analyze_one_shadow(
            img_bgr, current_shadow, L, gx, gy,
            expand=expand,
            color_diff=color_diff,
            darkest_frac=darkest_frac,
            shrink=shrink,
            min_bright_ref=min_bright_ref,
        ...
        if shadow_feats is not None:
            per_shadow_features.append(shadow_feats)
            log_ratios.append(log_ratio)
    ...
    if usable >= 2:
        # Build feature matrix: num_shadows x 4 features
        feature_matrix = np.array([[f['sr_median'], f['sr_iqr'], f['sr_skew'], f['sr_kurtosis']] 
                                   for f in per_shadow_features], dtype=np.float32)
        
        # Pairwise distances in Z-space (all 4 SR features)
        ...
        # Scale MAD to approximate std dev: std ≈ 1.4826 * MAD
        if mad_ratio > 1e-6:
...
def analyze_lighting(image_path, show_debug=False):
    ...
    score, ml_features = calculate_light_tamper_score(img, mask, debug=show_debug, viz=viz)
...
if __name__ == "__main__":
    analyze_lighting("data/images/20-edited.jpg", show_debug=True)